...
A project directory named `java-is-leap-year` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-is-leap-year` directory which is created just now.
